...
class Active2DStructure:
    ...
    def run_simulation(self, job_name, save_CAE):
        ...
        if save_CAE == True:
            mdb.saveAs(pathName='CAE'+job_name)
            #return True
    ...
    def csv_output(aborted, job_name):
        ...
        out = os.path.join(path_current,'outs-' + job_name.split('.')[0] + '.csv')
        # out = os.path.join(os.path.dirname(path_current),'CSV_island','outs-' + job_name.split('.')[0] + '.csv') # PACE large dataset
...
"""
# This CAE version:
# num_cpus = 1 for local
# frames 1-5 considered, so active_strain reduced and some other parameters modified
#     following descriptions in main(), except that minInc is set very small.
# "outs" (multiple frames) CSV generated, in the current directory.
# save_CAE = True (caeName is 'CAE'+job_name)
"""

def main(gene_file, job_name):
    # Parameters given here, not in a separate file
    
    # number of cpus, 1 or other values
    num_cpus = 1
