function [distTable, distML, distFE,out_Optim_ML,out_Optim_FE] = ShapeDist(optimType,BCType, ...
...
surf(out_Optim_ML(:,:,1),out_Optim_ML(:,:,2),out_Optim_ML(:,:,3),...
...
% light;
camlight; 
material([0.4 0.5 0.7]); lighting gouraud;
colormap(viridis)
...
% ====== Distance measure ======
% dist calculation
TOx = double(outIntuiOri(:,:,1));
TOy = double(outIntuiOri(:,:,2));
TOz = double(outIntuiOri(:,:,3));
% TOT = delaunay(TOx,TOy);
TOT = getConnectivity(TOx,TOy,TOz);
TO = triangulation(TOT,TOx(:),TOy(:),TOz(:));

QP1 = double(reshape(out_Optim_ML,[],3));
distML = point2trimesh('Faces',TO.ConnectivityList, ...
    'Vertices',TO.Points, 'QueryPoints',QP1, 'Algorithm','vectorized');
QP2 = double(reshape(out_Optim_FE,[],3));
distFE = point2trimesh('Faces',TO.ConnectivityList, ...
    'Vertices',TO.Points, 'QueryPoints',QP2, 'Algorithm','vectorized');
...
if ~isempty(csv_Optim)
    figure();
    distFE_perc = abs(distFE)/refL*100;
    s = pcolor(reshape(distFE_perc,16,16));
    cb=colorbar; 
    cb.Label.String='Relative distance(%)';
    s.FaceColor = 'interp';
    s.EdgeColor = 'w';
    title(['Max ',num2str(max(distFE_perc),3),'%, Avg ',num2str(mean(distFE_perc),3),'%']);
    set(gca,'FontName','Arial','FontSize',15); axis off;
    colormap(viridis);
end
...
% dist table  
Objects = ["Target vs. ML-optim"; "Target vs. FE-optim"];
distAvg = NaN(2,1);
distMax = NaN(2,1);
distRMS = NaN(2,1);
...
if ~isempty(csv_Optim)
    distAvg(2) = mean(abs(distFE));
    distMax(2) = max(abs(distFE));
    distRMS(2) = rms(distFE);
end
