...
% put together
outxy = cat(3,outx,outy);
clear outx outy outz
...
f5_XYZValidxy = outxy(:,:,:,numTrainSet+1:end); % 
...
load('mat_netD2s_lgraph_RN11_twocoords.mat');
...
option1 = trainingOptions('adam', ...
    ...
    'Shuffle','every-epoch', ...
    'ValidationData',{insValid,f5_XYZValidxy}, ...
...
%%
tic
[netD2s_f5xyz_xy,info] = trainNetwork(inBinary(:,:,:,1:numTrainSet),...
    outxy(:,:,:,1:numTrainSet),lgraph,option1);
fprintf('Net training completed. This step takes %f seconds!\n', toc) 

tic
save('mat_netD2s_f5convBCxyzEnd_RN22xy_51conv_15epoDrop.mat','netD2s_f5xyz_xy','info',...
    'mu','mux','muy','muz','sig','sigx','sigy','sigz',...
    'numData','numTrainSet','miniBatchSize',...
    'insValid','f5_XYZValidxy','-v7.3');
